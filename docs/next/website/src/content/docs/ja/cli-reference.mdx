...
`stop-if-idle` は、セッションと処理中の作業の確認からシャットダウンの確定までをサーバー上でアトミックに実行します。
ペイン、保持されたターミナル状態、分離実行中のコマンド、プラグインコマンド、またはワークツリー操作が残っている場合は、停止せずに `server_busy` を返します。
ソケットメソッドは空のパラメーターを取る `server.stop_if_idle` で、ping のオプション機能 `stop_if_idle` で対応を通知します。
古いサーバーはこの操作に対応しておらず、コマンドが無条件の `stop` にフォールバックすることはありません。
成功レスポンスはシャットダウンが確定したことを意味し、その後に監視プロセスが新しいサーバーを起動する場合があります。
