...
| Agent | `agent.list`, `agent.get`, `agent.read`, `agent.explain`, `agent.send_keys`, `agent.prompt`, `agent.wait`, `agent.rename`, `agent.focus`, `agent.start`, `agent.view.get`, `agent.view.set`, `agent.view.clear` |
...
`herdr api projection [--watch]` reads a narrow endpoint projection containing the runtime boot ID, native agent ordering, optional view label, and custom tab-label flags.
Each JSON line has `type: "runtime.projection"`, a native `revision`, and a `projection` object.
The command requires the runtime to advertise `passive_metadata` through both the API and endpoint handshake; older runtimes are rejected before endpoint attachment.
It does not activate a surface, create a workspace, change terminal geometry, or forward input.
Watch mode emits replacements only when these projected facts change and ends when stdin closes or receives input.
The initial projection has a five-second deadline.
The API and endpoint socket overrides must select the same runtime.
Consumers should prefer public `session.snapshot` and `agent.view.get` data when available and must not infer a view definition or source from the projection label.
...
Read the active definition and its evaluated order with `agent.view.get`:

```json
{"id":"view_get","method":"agent.view.get","params":{}}
```

The response has `type: "agent_view_state"` and a `view` object containing `definition` and ordered public `pane_ids`.
A null definition means no override; the pane IDs then reflect the runtime's ordinary agent ordering.
`session.snapshot` includes the same object in optional `agent_view` metadata.
An absent field means the runtime does not support this metadata.
Subscribe explicitly to `agent.view.changed` to observe definition changes, including source-owned clearing when a plugin is disabled or removed.
Refresh the projection after ordinary pane, workspace, focus, and agent-status events too, because view membership can depend on those facts.
Tab metadata also includes optional `custom_label` to distinguish explicit labels from automatic names without guessing on older runtimes.
