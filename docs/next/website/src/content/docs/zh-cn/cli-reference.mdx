...
`stop-if-idle` 要求服务器以原子方式检查会话和待完成的工作并提交关闭。
如果仍有窗格、保留的终端状态、分离运行的命令、插件命令或工作树操作,则返回 `server_busy` 而不停止服务器。
套接字方法为 `server.stop_if_idle`,参数为空,通过 ping 中可选的 `stop_if_idle` 能力字段声明支持。
旧服务器不支持此操作,该命令不会回退到无条件的 `stop`。
成功响应表示关闭已提交;监管进程随后可能启动新服务器。
