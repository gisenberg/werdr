...
export class TerminalController {
  ...
  get cellMetrics() { return this.terminal?.renderer?.getMetrics(); }
  commandSelection() { return this.copyMode?.active ? this.copyMode.commandSelection() : this.selectionMode?.commandSelection(); }
  ...
  async connect(takeover = false, retry = false) {
    ...
    const fitVisible = () => {
      ...
      // reserves its own cells and has no pane scrollbar.
      const metrics = this.target ? term.renderer?.getMetrics() : undefined;
    ...
    let applyingFrame = false, scrollReady = !!this.target, settled = false;
