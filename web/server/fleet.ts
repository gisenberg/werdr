...
const validWorkspaceRows = (workspace: any): boolean => workspace && (workspace.custom_label === undefined || typeof workspace.custom_label === 'boolean') && (workspace.branch === undefined || typeof workspace.branch === 'string') && (workspace.git_ahead_behind === undefined || Array.isArray(workspace.git_ahead_behind) && workspace.git_ahead_behind.length === 2 && workspace.git_ahead_behind.every((count: unknown) => Number.isSafeInteger(count) && (count as number) >= 0));
...
export class Fleet extends EventEmitter {
  ...
  private async open(host: Host) {
    ...
    try {
      ...
      host.commandWatch.reconcile(api);
