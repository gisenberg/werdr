...
export async function settingsStore(path: string) {
  ...
  if (stored !== undefined && (!stored || typeof stored !== 'object' || stored.preferences === undefined || ![1, 2, 3, 4, 5, 6].includes(stored.version) || !Number.isSafeInteger(stored.revision) || stored.revision < 0)) throw new Error('Invalid or unsupported browser settings store');
  ...
  if (stored && stored.version < 6) await writePrivateJson(path, { version: 6, ...state });
  ...
  return {
    ...
    update(revision: unknown, preferences: unknown): Promise<SettingsState> {
      ...
      const task = queue.then(async () => {
        ...
        await writePrivateJson(path, { version: 6, ...next }); state = next; return structuredClone(state);
      });
