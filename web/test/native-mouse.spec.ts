...
test('native mouse modes control browser gestures and release outside the pane', async ({ page }) => {
  ...
  const inputs: string[] = []; let enabled = false;
  try {
    await page.routeWebSocket('**/ws/terminal?*', socket => {
      ...
      socket.onMessage(message => { const record = JSON.parse(String(message)); if (record.type === 'terminal.input' && record.text.startsWith('\x1b[<')) inputs.push(record.text); else server.send(message); });
    });
