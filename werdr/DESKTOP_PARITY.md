...
| Agents | Start/prompt/rename, native ordering/views, detail/status metadata and attention navigation | Partial: basic actions and fleet list plus next/previous blocked-agent navigation in native snapshot order, scoped by host and excluding unavailable hosts; unit and browser tests verify wrapping, focus restoration and live state changes; detailed native views remain |
